...
class DList:
  ...
  def sort(self):
      ...
      length=len(self)
      counter=0
      l.addLast(self.head.elem)
      self.removeFirst()
      if self.getAt(0)<l.head.elem:
          l.addFirst(self.getAt(0))
          self.removeFirst()
      else:
          l.addLast(self.getAt(0))
          self.removeFirst()
      while counter<length-2: 
              len1=len(l)
              count=0
              while len1==len(l):
                  if self.getAt(counter)<l.getAt(count):
                      l.insertAt(count,self.getAt(counter))

                  else:
                      if count<len1-1:
                          count+=1
                      else:
                          l.addLast(self.getAt(counter))
